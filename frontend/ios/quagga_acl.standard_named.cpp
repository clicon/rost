...
(<action:string  keyword:deny>("Specify packets to reject")|<action:string  keyword:permit>("Specify packets to forward")) {
    ...
    <prefix:ipv4prefix>("Prefix to match. e.g. 10.0.0.0/8"), cli_set("ipv4.access-list.standard.named[].line[] $!name=quagga::quagga_acl_cb() $_SEQ=(sequence)10 $!action=$action $!src=$prefix $!exact_match=(bool)false"), ADMIN;
    <prefix:ipv4prefix>("Prefix to match. e.g. 10.0.0.0/8") exact-match("Exact match of the prefixes"), cli_set("ipv4.access-list.standard.named[].line[] $!name=quagga::quagga_acl_cb() $_SEQ=(sequence)10 $!action=$action $!src=$prefix $!exact_match=(bool)true"), ADMIN;
...
no("Negate a command or set its defaults") {
    (<action:string  keyword:deny>("Specify packets to reject")|<action:string  keyword:permit>("Specify packets to forward")) {
        ...
        <prefix:ipv4prefix>("Prefix to match. e.g. 10.0.0.0/8"), cli_del("ipv4.access-list.standard.named[].line[] $!name=quagga::quagga_acl_cb() $_SEQ=(sequence)10 $!action=$action $!src=$prefix $!exact_match=(bool)true"), ADMIN;
        <prefix:ipv4prefix>("Prefix to match. e.g. 10.0.0.0/8") exact-match("Exact match of the prefixes"), cli_del("ipv4.access-list.standard.named[].line[] $!name=quagga::quagga_acl_cb() $_SEQ=(sequence)10 $!action=$action $!src=$prefix $!exact_match=(bool)true"), ADMIN;
