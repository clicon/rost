...
no("Negate a command or set its defaults") neighbor("Specify a neighbor router") (<neighbor:ipv4addr expand_bgp_neighbor()>("Neighbor address")|<neighbor:ipv4addr show:A.B.C.D>("Neighbor address")) @IOS_BGP_NONEIGHBOR, cli_del(), ADMIN;

/* PEER-GROUP */
neighbor("Specify a neighbor router") (<peergroup:string show:WORD>("Neighbor tag")|<peergroup:string expand_bgp_peergroup()>("Neighbor tag")) @IOS_BGP_PEERGROUP, cli_set(), ADMIN;
