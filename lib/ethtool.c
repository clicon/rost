...
int
rost_ethtool_gset(char *ifname, struct ethtool_cmd *ep)
{
  int fd;
  int ret = -1;
  struct ifreq ifr;
 ...
 */
struct rost_etstats *
rost_ethtool_gstats(char *ifname, const char *label)
{
    int i;
    int fd;
    int nstats;
    ...
    etsp = (struct rost_etstats *)
	chunk(sizeof(struct rost_etstats) +
	      (nstats * sizeof(struct rost_etstatent)),
	      label);
    if (etsp == NULL) {
	clicon_err(OE_UNIX, errno, "failed to allocate memory");
	goto catch;
...
int
rost_ethtool_stat(struct rost_etstats *ets, char *name)
{
  int i;
...
int
rost_ethtool_getlink(const char *ifname, int *link)
{
    int fd;
    int retval = -1;
    struct ifreq ifr;
...
int
rost_ethtool_drvinfo(const char *ifname, struct ethtool_drvinfo *drvinfo)
{
    int fd;
    int retval = -1;
    struct ifreq ifr;
 ...
 */
int
rost_ifspeed(char *ifname, uint32_t *speed)
{
